...
export default function InterestedPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const mapped = (props ?? []).map(p => ({ ...p, my_card_name: listingMap[p.listing_id] ?? '' }))
      setProposals(mapped)
  ...
  const openReviewModal = (p: any) => {
    ...
    setComment('')
    setReviewDone(false)
    setReviewModal({ proposalId: p.id, revieweeId: p.proposer_id })
  }
              ...
              <div key={p.id} className="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                {p.offer_type === 'card' && (
                  <div className="flex gap-3 items-center bg-gray-50 rounded-xl p-3 mb-3">
                    {p.offered_card_image && <img src={p.offered_card_image} alt={p.offered_card_name} className="w-12 rounded-lg flex-shrink-0" />}
                    <div>
                ...
                {p.status === 'accepted' && (
                  <div className="mt-3 flex items-center justify-between gap-3 bg-green-50 rounded-xl p-3">
                    <p className="text-sm text-green-700 font-medium">已接受，請雙方聯繫完成交換！</p>
